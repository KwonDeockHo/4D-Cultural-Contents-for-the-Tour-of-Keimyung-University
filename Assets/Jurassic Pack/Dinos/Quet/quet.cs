...
public class quet : MonoBehaviour
{
	...
	const float MAXYAW=15, MAXPITCH=6, MAXCROUCH=4, MAXANG=2, TANG=0.025f;
	...
	void Start()
	{
		Root = transform.Find ("Quet/root");
		
		Left_Hips = transform.Find ("Quet/root/low spine0/low spine1/pelvis/left hips");
		Right_Hips = transform.Find ("Quet/root/low spine0/low spine1/pelvis/right hips");
		Left_Leg  = transform.Find ("Quet/root/low spine0/low spine1/pelvis/left hips/left leg");
		Right_Leg = transform.Find ("Quet/root/low spine0/low spine1/pelvis/right hips/right leg");
		Left_Foot = transform.Find ("Quet/root/low spine0/low spine1/pelvis/left hips/left leg/left foot");
		Right_Foot = transform.Find ("Quet/root/low spine0/low spine1/pelvis/right hips/right leg/right foot");
		
		
		Right_Wing0 = transform.Find ("Quet/root/spine0/spine1/spine2/right wing0");
		Left_Wing0 = transform.Find ("Quet/root/spine0/spine1/spine2/left wing0");
		Right_Wing1 = transform.Find ("Quet/root/spine0/spine1/spine2/right wing0/right wing1/right wing2");
		Left_Wing1 = transform.Find ("Quet/root/spine0/spine1/spine2/left wing0/left wing1/left wing2");
		Right_Hand = transform.Find ("Quet/root/spine0/spine1/spine2/right wing0/right wing1/right wing2/right wing3");
		Left_Hand = transform.Find ("Quet/root/spine0/spine1/spine2/left wing0/left wing1/left wing2/left wing3");
		
		Neck0 = transform.Find ("Quet/root/spine0/spine1/spine2/neck0");
		Neck1 = transform.Find ("Quet/root/spine0/spine1/spine2/neck0/neck1");
		Neck2 = transform.Find ("Quet/root/spine0/spine1/spine2/neck0/neck1/neck2");
		Neck3 = transform.Find ("Quet/root/spine0/spine1/spine2/neck0/neck1/neck2/neck3");
		Neck4 = transform.Find ("Quet/root/spine0/spine1/spine2/neck0/neck1/neck2/neck3/neck4");
		Neck5 = transform.Find ("Quet/root/spine0/spine1/spine2/neck0/neck1/neck2/neck3/neck4/neck5");
		Neck6 = transform.Find ("Quet/root/spine0/spine1/spine2/neck0/neck1/neck2/neck3/neck4/neck5/neck6");
		Head  = transform.Find ("Quet/root/spine0/spine1/spine2/neck0/neck1/neck2/neck3/neck4/neck5/neck6/head");
	...
	//*************************************************************************************************************************************************
	//Check collisions
	void OnCollisionStay(Collision col) { shared.ManageCollision(col, MAXPITCH, MAXCROUCH, source, Quet1, Hit_jaw, Hit_head, Hit_tail); }
	...
	void PlaySound(string name, int time)
	{
		if((time==shared.currframe)&&(shared.currframe!=shared.lastframe))
		{
			...
			case "Bite": source[1].pitch=Random.Range(1.5f, 1.75f); source[1].PlayOneShot(Bigstep, 0.5f);
				shared.lastframe=shared.currframe; break;
			case "Sniff": source[1].pitch=Random.Range(1.5f, 1.75f);
				...
				shared.lastframe=shared.currframe; break;
			case "Die": source[1].pitch=Random.Range(1.0f, 1.25f); source[1].PlayOneShot(shared.IsOnWater?Largesplash:Bigstep, 1.0f);
				shared.lastframe=shared.currframe; shared.IsDead=true; break;
			case "Eat": source[0].pitch=Random.Range(1.0f, 1.25f); source[0].PlayOneShot(Swallow, 0.1f);
				shared.lastframe=shared.currframe; break;
			...
			case "Atk": int rnd1 = Random.Range(0, 2); source[0].pitch=Random.Range(1.0f, 1.25f);
				...
				else source[0].PlayOneShot(Quet3, 1.0f);
				shared.lastframe=shared.currframe; break;
	...
	void FixedUpdate ()
	{
		...
		{
			body.mass=5; body.drag=1; body.angularDrag=1; anm.speed=shared.AnimSpeed/2; anm.Play("Quet|Run");
		...
		//Stopped
		if(NextAnm.IsName("Quet|IdleA") | CurrAnm.IsName("Quet|IdleA") | CurrAnm.IsName("Quet|Die1") | CurrAnm.IsName("Quet|Die2") | CurrAnm.IsName("Quet|Fall"))
		{
			if(CurrAnm.IsName("Quet|Die1")) { reset=true; shared.IsConstrained=true; if(!shared.IsDead) { PlaySound("Growl", 1); PlaySound("Die", 11); } }
			else if(CurrAnm.IsName("Quet|Die2"))
			{
				reset=true; shared.IsConstrained=true; body.velocity = new Vector3(0, 0, 0); 
				if(!shared.IsDead) PlaySound("Die", 0);
			}
			else if(CurrAnm.IsName("Quet|Fall"))
			{
				reset=true; shared.IsFlying=true;
				body.AddForce(-Vector3.up*Mathf.Lerp(dir.y, 64, 1.0f));
				if(CurrAnm.normalizedTime<0.1f) source[0].PlayOneShot(Quet2, 1.0f);
			} 
		}
		
		//Forward
		else if(NextAnm.IsName("Quet|Walk") | CurrAnm.IsName("Quet|Walk"))
		{
		...
		//Running
		else if(NextAnm.IsName("Quet|Run") | CurrAnm.IsName("Quet|Run"))
		{
		...
		//Strafe/Turn right
		else if(NextAnm.IsName("Quet|Strafe+") | CurrAnm.IsName("Quet|Strafe+"))
		{
			transform.rotation*= Quaternion.Euler(0, anm.GetFloat("Turn")*2, 0);
		...
		//Takeoff
		else if(CurrAnm.IsName("Quet|Takeoff"))
		{
			shared.IsFlying=true;
		...
		//Run to Fly
		else if(NextAnm.IsName("Quet|RunToFlight") | CurrAnm.IsName("Quet|RunToFlight"))
		{
			shared.IsFlying=true;
		...
		//Fly to Run
		else if(NextAnm.IsName("Quet|FlightToRun") | CurrAnm.IsName("Quet|FlightToRun"))
		{
			shared.IsFlying=true;
			body.AddForce(transform.forward*100*transform.localScale.x*anm.speed);
			PlaySound("Step", 5); PlaySound("Step", 6); PlaySound("Sniff", 7); PlaySound("Sniff", 8);
		...
		//Fly
		else if(NextAnm.IsName("Quet|Flight") | CurrAnm.IsName("Quet|Flight") | NextAnm.IsName("Quet|FlightGrowl") | CurrAnm.IsName("Quet|FlightGrowl") |
		   NextAnm.IsName("Quet|Glide") | CurrAnm.IsName("Quet|Glide") | NextAnm.IsName("Quet|GlideGrowl") | CurrAnm.IsName("Quet|GlideGrowl"))
		{
			shared.IsFlying=true;
			...
			transform.rotation*= Quaternion.Euler(0, anm.GetFloat("Turn")*2, 0);
			body.AddForce(-Root.right*(60+Mathf.Abs(anm.GetFloat("Pitch")*40))*transform.localScale.x);
			if(body.velocity.magnitude!=0) body.AddForce(-Vector3.up*Mathf.Lerp(dir.y, 64/body.velocity.magnitude, 1.0f));

			if(CurrAnm.IsName("Quet|Flight")) { PlaySound("Sniff", 5); PlaySound("Sniff", 6); }
			else if(CurrAnm.IsName("Quet|FlightGrowl")) { PlaySound("Atk", 2); PlaySound("Sniff", 5); }
			else if(CurrAnm.IsName("Quet|GlideGrowl")) PlaySound("Growl", 2);
		}
		
		//Fly - Stationary
		else if(CurrAnm.IsName("Quet|Statio") | CurrAnm.IsName("Quet|StatioGrowl") | CurrAnm.IsName("Quet|IdleD") | CurrAnm.IsName("Quet|FlyAtk"))
		{
			shared.IsFlying=true;
			...
			if(shared.IsOnGround&&CurrAnm.IsName("Quet|FlyAtk")) body.AddForce(Vector3.up*50*transform.localScale.x); //takeoff
			...
			if(CurrAnm.IsName("Quet|StatioGrowl")) PlaySound("Atk", 2);
			else if(CurrAnm.IsName("Quet|IdleD")) { PlaySound("Atk", 2); PlaySound("Step", 10); }
			else if(CurrAnm.IsName("Quet|FlyAtk")) { shared.IsAttacking=true; PlaySound("Atk", 3); PlaySound("Bite", 8); }
			else { PlaySound("Sniff", 5); PlaySound("Sniff", 6); }
		}
	
		//Various
		else if(CurrAnm.IsName("Quet|Landing")) { shared.IsFlying=true; PlaySound("Step", 2); PlaySound("Step", 3); }
		else if(CurrAnm.IsName("Quet|IdleB")) { PlaySound("Atk", 2); PlaySound("Bite", 5); PlaySound("Bite", 8); PlaySound("Bite", 10); }
		else if(CurrAnm.IsName("Quet|IdleC")) { reset=true; shared.IsConstrained=true; }
		else if(CurrAnm.IsName("Quet|EatA")) { reset=true; PlaySound("Eat", 1); }
		else if(CurrAnm.IsName("Quet|EatB")) { reset=true; PlaySound("Step", 0); }
		else if(CurrAnm.IsName("Quet|EatC")) reset=true;
		else if(CurrAnm.IsName("Quet|ToSleep")){ reset=true; shared.IsConstrained=true; }
		else if(CurrAnm.IsName("Quet|Sleep")) { reset=true; shared.IsConstrained=true; PlaySound("Sleep", 1); }
		else if(CurrAnm.IsName("Quet|Die-")) { shared.IsConstrained=true; PlaySound("Atk", 2); transform.tag=("Dino"); shared.IsDead=false; }

		//Play wind sound based on speed
		if(shared.IsFlying)
		{
			if(!source[2].isPlaying) source[2].PlayOneShot(Wind);
			source[2].volume=body.velocity.magnitude/(100*transform.localScale.x);
			source[2].pitch=body.velocity.magnitude/(50*transform.localScale.x);
		}
	...
	void LateUpdate()
	{
		...
		//Wings
		Right_Wing0.transform.rotation*= Quaternion.Euler(flyRoll/2, 0, Mathf.Clamp(-flyPitch, -35, 0));
		Left_Wing0.transform.rotation*= Quaternion.Euler(flyRoll/2, 0, Mathf.Clamp(flyPitch, 0, 35));
		Right_Wing0.GetChild(0).transform.rotation*= Quaternion.Euler(0, Mathf.Clamp(-flyRoll, -15, 10), Mathf.Clamp(flyPitch, 0, 90)+Mathf.Abs(flyRoll)/2);
		Left_Wing0.GetChild(0).transform.rotation*= Quaternion.Euler(0, Mathf.Clamp(flyRoll, -15, 10), Mathf.Clamp(-flyPitch, -90, 0)-Mathf.Abs(flyRoll)/2);
