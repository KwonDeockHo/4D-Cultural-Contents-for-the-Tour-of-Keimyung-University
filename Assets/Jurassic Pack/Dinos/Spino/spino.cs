...
public class spino : MonoBehaviour
{
	...
	void Start()
	{
		...
		Right_Foot1 = transform.Find ("Spino/root/tail0/tail1/right hips/right leg/right foot0/right foot1");
		...
		Left_Foot1 = transform.Find ("Spino/root/tail0/tail1/left hips/left leg/left foot0/left foot1");
	...
	//*************************************************************************************************************************************************
	//Check collisions
	void OnCollisionStay(Collision col) { shared.ManageCollision(col, MAXPITCH, MAXCROUCH, source, Spino3, Hit_jaw, Hit_head, Hit_tail); }
	...
	void PlaySound(string name, int time)
	{
		if((time==shared.currframe)&&(shared.currframe!=shared.lastframe))
		{
			...
			case "Atk": int rnd1 = Random.Range (0, 2); source[0].pitch=Random.Range(1.0f, 1.25f);
				if(rnd1==0)source[0].PlayOneShot(Spino3, 1.0f);
				else source[0].PlayOneShot(Spino4, 1.0f);
				shared.lastframe=shared.currframe; break;
	...
	void FixedUpdate ()
	{
		...
		{
			body.mass=5; body.drag=1; body.angularDrag=1; anm.speed=shared.AnimSpeed/2; anm.Play("Spino|Run");
		...
		{
			if(CurrAnm.IsName("Spino|Die1")) { reset=true; shared.IsConstrained=true; if(!shared.IsDead) { PlaySound("Atk", 2); PlaySound("Die", 12); } }
			else if(CurrAnm.IsName("Spino|Die2")) { reset=true; shared.IsConstrained=true; if(!shared.IsDead) { PlaySound("Atk", 2); PlaySound("Die", 10); } }
		}
		...
		{
			PlaySound("Step", 9);
			if(CurrAnm.IsName("Spino|StepAtk1") | CurrAnm.IsName("Spino|StepAtk2")) shared.IsAttacking=true;
		}
		   ...
		   (CurrAnm.IsName("Spino|ToEatC") && CurrAnm.normalizedTime < 0.5))
		{
			...
			if(anm.GetCurrentAnimatorStateInfo(0).IsName("Spino|WalkGrowl")) { PlaySound("Growl", 1); PlaySound("Step", 6); PlaySound("Step", 13); }
			else if(anm.GetCurrentAnimatorStateInfo(0).IsName("Spino|Walk")) { PlaySound("Step", 6); PlaySound("Step", 13); }
		   ...
		   (CurrAnm.IsName("Spino|StepAtk2") && CurrAnm.normalizedTime < 0.6))
		{
			...
			if(anm.GetCurrentAnimatorStateInfo(0).IsName("Spino|RunGrowl")) { PlaySound("Growl", 1); PlaySound("Step", 6); PlaySound("Step", 13); }
			else if(anm.GetCurrentAnimatorStateInfo(0).IsName("Spino|Run")) { PlaySound("Step", 6); PlaySound("Step", 13); }
			else if(CurrAnm.IsName("Spino|StepAtk1") | CurrAnm.IsName("Spino|StepAtk2")) { shared.IsAttacking=true; PlaySound("Atk", 2); PlaySound("Bite", 5); }
		...
		//Various
		else if(CurrAnm.IsName("Spino|EatA")) { reset=true; PlaySound("Eat", 4); PlaySound("Bite", 5); }
		else if(CurrAnm.IsName("Spino|EatB") | CurrAnm.IsName("Spino|EatC")) reset=true;
		else if(CurrAnm.IsName("Spino|Sleep")) { reset=true; shared.IsConstrained=true; PlaySound("Sleep", 2); }
		else if(CurrAnm.IsName("Spino|ToSleep1") | CurrAnm.IsName("Spino|ToSleep2")) { reset=true; shared.IsConstrained=true; }
		...
		else if(CurrAnm.IsName("Spino|Idle2B")) { reset=true; PlaySound("Bite", 4); PlaySound("Bite", 6); PlaySound("Bite", 8);}
		else if(CurrAnm.IsName("Spino|Idle2C")) PlaySound("Growl", 2);
		else if(CurrAnm.IsName("Spino|Idle2D")) { reset=true; PlaySound("Atk", 2); }
		...
		else if(CurrAnm.IsName("Spino|Die1-")) { shared.IsConstrained=true; PlaySound("Growl", 3); transform.tag=("Dino"); shared.IsDead=false; }
		else if(CurrAnm.IsName("Spino|Die2-")) { shared.IsConstrained=true; PlaySound("Growl", 3); transform.tag=("Dino"); shared.IsDead=false; }
	}
