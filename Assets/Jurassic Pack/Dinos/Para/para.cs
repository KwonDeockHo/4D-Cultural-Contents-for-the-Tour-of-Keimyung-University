...
public class para : MonoBehaviour
{
	...
	const float MAXYAW=20, MAXPITCH=15, MAXCROUCH=4, MAXANG=1.5f, TANG=0.025f;
	...
	//*************************************************************************************************************************************************
	//Check collisions
	void OnCollisionStay(Collision col) { shared.ManageCollision(col, MAXPITCH, MAXCROUCH, source, Para1, Hit_jaw, Hit_head, Hit_tail); }
	...
	void FixedUpdate ()
	{
		...
		{
			body.mass=5; body.drag=1; body.angularDrag=1; anm.speed=shared.AnimSpeed/2; anm.Play("Para|Run");
		...
		else if(CurrAnm.IsName("Para|Die1-")) { PlaySound("Growl", 3); transform.tag=("Dino"); shared.IsDead=false; }
		else if(CurrAnm.IsName("Para|Die2-")) { PlaySound("Growl", 3); transform.tag=("Dino"); shared.IsDead=false; }
	}
