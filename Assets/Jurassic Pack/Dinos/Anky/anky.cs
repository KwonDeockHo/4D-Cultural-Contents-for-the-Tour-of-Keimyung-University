...
public class anky : MonoBehaviour
{
	...
	//***********************************************************************************************************************************************************************************************************
	//Check collisions
	void OnCollisionStay(Collision col) { shared.ManageCollision(col, MAXPITCH, MAXCROUCH, source, Anky2, Hit_jaw, Hit_head, Hit_tail); }
	...
	void PlaySound(string name, int time)
	{
		if((time==shared.currframe)&&(shared.currframe!=shared.lastframe))
		{
			...
			case "Hit": source[1].pitch=Random.Range(1.0f, 1.25f); source[1].PlayOneShot(shared.IsOnWater?Largesplash:Largestep, 1.0f);
							shared.lastframe=shared.currframe; break;
			case "Die": source[0].pitch=Random.Range(1.0f, 1.25f); source[1].PlayOneShot(shared.IsOnWater?Largesplash:Largestep, 1.0f);
	...
	void FixedUpdate()
	{
		...
		{
			body.mass=5; body.drag=1; body.angularDrag=1; anm.speed=shared.AnimSpeed/2; anm.Play("Anky|Run");
		...
		else if(CurrAnm.IsName("Anky|Die1-")) { PlaySound("Growl", 3); transform.tag=("Dino"); shared.IsDead=false; }
		else if(CurrAnm.IsName("Anky|Die2-")) { PlaySound("Growl", 3); transform.tag=("Dino"); shared.IsDead=false; }
	}
