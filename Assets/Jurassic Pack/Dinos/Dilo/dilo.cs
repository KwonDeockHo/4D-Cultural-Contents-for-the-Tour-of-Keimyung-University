...
public class dilo : MonoBehaviour
{
	...
	//*************************************************************************************************************************************************
	//Check collisions
	void OnCollisionStay(Collision col) { shared.ManageCollision(col, MAXPITCH, MAXCROUCH, source, Dilo3, Hit_jaw, Hit_head, Hit_tail); }
	...
	void FixedUpdate()
	{
		...
		{
			body.mass=5; body.drag=1; body.angularDrag=1; anm.speed=shared.AnimSpeed/2; anm.Play("Dilo|Run"); shared.Health-=0.1f;
		...
		//Various
		else if(CurrAnm.IsName("Dilo|IdleAtk3")) { shared.IsAttacking=true; transform.rotation*= Quaternion.Euler(0, anm.GetFloat("Turn")*2, 0); PlaySound("Growl", 1); }
		...
		{ shared.IsAttacking=true; transform.rotation*= Quaternion.Euler(0, anm.GetFloat("Turn")*2, 0); PlaySound("AtkA", 2); PlaySound("Bite", 9); }
		...
		else if(CurrAnm.IsName("Dilo|EatA")) { reset=true; PlaySound("Eat", 1); }
		...
		else if(CurrAnm.IsName("Dilo|Die-")) { reset=true; PlaySound("AtkA", 1); transform.tag=("Dino"); shared.IsDead=false; }
	}
